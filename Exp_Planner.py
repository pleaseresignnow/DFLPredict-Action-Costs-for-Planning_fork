...
parser.add_argument("--sas_file", type=str, help="name of the sas file", default= 'problem_layout.sas', required= False)
...
parser.add_argument("--lambda_val", type=int, help="degree of misspecifaction", default=1, required= False)
...
val_planner = [val_planner1, val_planner2]
...
validation_setsize = len(validation_dataset)
test_setsize = len(test_dataset)

batch_size = args.batchsize

train_dl = DataLoader( train_dataset , batch_size=batch_size, num_workers= num_workers)
validation_dl = DataLoader( validation_dataset, batch_size=25, num_workers= num_workers)
test_dl = DataLoader( test_dataset, batch_size=test_setsize, num_workers= num_workers)
...
if use_heuristics:
    ### Must Run inside `DFL-FD/builds/release/bin`
    ### ./downward --search "dfl_h_server([ff()])" --internal-plan-file sas_plan  < ../../../../benchmarks/rovers/p01.sas`
    ### ./downward --search 'eager_wastar([lmcut()], w=2)' --internal-plan-file sas_plan  < ../../../../benchmarks/rovers/p01.sas`
    planner._heuristics_on()
    heuristicname = args.heuristicname

## Initially used to Plot
...
def run_exp_script(model , config, name):
    ...
    printed_name = printed_name+'-lambda{}'.format(lambda_val)

    printed_name = "Training"+ printed_name+'-Deg{}'.format(deg) 
    printed_name = 'sas'+ sas_file.split('/')[-1] + "-"+printed_name
