  ...
  void write(ostream & o) const
  {
	...
	for(int j = 0; j<rovers[i].travs.size();++j){
	  ...
	  if (probtype == RoverDescriptor::CSTRIPS) {
	    o.setf(ios::fixed);
	    o.precision(1);
	    o << "\t(= (traverse_cost rover" << i << " waypoint" <<  from << " waypoint" << to << ") " << (int)round(m.distance(from, to)*(drnd()+0.5)* 10) << ")\n";
	  }
	}
...
RoverDescriptor commandLine(int & seed,string & filename,int argc, char * argv[])
{
  ...
  if(nxt < 5) usage();
	...
	cout << ";; seed: " << seed << endl;
	cout << ";; numRovers: " << val[0] << endl;
	cout << ";; numWaypoints: " << val[1] << endl;
	cout << ";; numObjectives: " << val[2] << endl;
	cout << ";; numCameras: " << val[3] << endl;
	cout << ";; numgoals: " << val[4] << endl << endl;
