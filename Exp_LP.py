...
from data import generate_instances, DataWrapper
...
train_dl = DataLoader(DataWrapper(x_train, y_train, sol_train), batch_size=batch_size, num_workers= num_workers)
validation_dl = DataLoader(DataWrapper(x_val, y_val,sol_val), batch_size=50, num_workers= num_workers)
test_dl = DataLoader(DataWrapper(x_test, y_test, sol_test), batch_size=test_setsize, num_workers= num_workers)
...
def run_exp_script(model , config, name):
    ...
    ax1.set_title ("Prediction of Action Cost")
...
if  name in ['SPO', 'MSE']:
    for with_relu in [True,  False]:
        for lr in [0.05]: # 
            for epochs in [20]:
                config = {"epochs": epochs,  "log_every_n_steps": 5, "batch_size":batch_size ,"lr": lr, "with_relu":with_relu }

                model = module_dict[name](net=build_net(input_dim, output_dim, with_relu= config['with_relu'] ),
                                        training_problem=lpsolver, evaluation_problem = val_planner, 
